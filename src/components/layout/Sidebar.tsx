...
export default function Sidebar() {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      {/* Closed state: 3 Stacked Titled Notches anchored at top-left edge below header */}
      <div className="fixed left-0 top-[56px] sm:top-[60px] z-50 flex flex-col gap-1 select-none pointer-events-auto">
        {spokeCategoryCards.map((card) => {
          const NotchIcon = card.icon
          return (
            <SheetTrigger
              key={`trigger-${card.id}`}
              className="group flex items-center gap-1.5 px-2.5 py-1.5 sm:px-3 sm:py-2 bg-gradient-to-r from-[#3b2416] via-[#2c1a0e] to-[#1e1008] text-amber-100 border-y border-r border-[#8c5a36] hover:border-[#b8860b] rounded-r-lg shadow-xl transition-all duration-200 cursor-pointer hover:pl-3 text-left opacity-95 hover:opacity-100"
            >
              {/* Gold Accent Strip */}
              <div className="w-1 h-4 sm:h-5 rounded-full bg-gradient-to-b from-[#d4af37] via-[#b8860b] to-[#8b6508] shrink-0" />
              <NotchIcon className="h-3.5 w-3.5 text-[#b8860b] group-hover:scale-110 transition-transform shrink-0" />
              <span className="text-[11px] sm:text-xs font-mono font-bold tracking-tight text-amber-100 truncate max-w-[100px] sm:max-w-none">
                {card.title}
              </span>
            </SheetTrigger>
          )
